...
import type { InternalsResponse, InternalMark, CourseInternal } from '@nimora/shared-types';

const INTERNALS_URL = 'https://ecampus.psgtech.ac.in/studzone/ContinuousAssessment/CAMarksView';
...
export class InternalsScraperService {
   ...
   */
  async scrapeInternals(rollno: string, password: string): Promise<InternalsResponse> {
  ...
  private extractInternals($: cheerio.CheerioAPI): CourseInternal[] {
    ...
    $('div.card, div.course-card, table tbody tr').each((_, element) => {
      ...
      const courseCode = el.find('.course-code, h5, td:first-child').first().text().trim();
      const courseName = el.find('.course-name, h6, td:nth-child(2)').first().text().trim();
      ...
      el.find('.test-mark, .internal-mark, td.mark').each((_, markEl) => {
        ...
        if (markMatch) {
          marks.push({
            ...
            percentage: (parseFloat(markMatch[1]) / parseFloat(markMatch[2])) * 100,
          });
        }
      });
